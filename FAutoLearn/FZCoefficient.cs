...
namespace FAutoLearn
{
    public class FZMath
    {
        ...
        public CenterLineResult FitLineMinWidth(
            List<Point2d> points)
        {
            ...
            double bestMinProj = 0.0;
            double bestMaxProj = 0.0;

            for (int i = 0; i < h; i++)
            {
                Point2d p1 = hull[i];
                Point2d p2 = hull[(i + 1) % h];
            ...
            double centerProj =
                (bestMinProj + bestMaxProj) * 0.5;

            return new CenterLineResult
            {
                Width = bestWidth,
